...
* **Numeric** data types are used to store numbers. There are several different numeric data types available, each with its own range and precision. The most common numeric data types are:
    ...
    * **DECIMAL** - Stores exact numbers with fixed precision and scale. For example, DECIMAL(5,2) can store values from -999.99 to 999.99. 
        * **Example:** `price DECIMAL(10,2)`.
    * **NUMERIC** - A synonym for DECIMAL; it stores exact numbers with fixed precision and scale. 
...
* **String** data types are used to store text. The most common string data types are:
    ...
    * **TEXT** - Stores variable-length non-Unicode text up to 2^31-1 bytes. It is deprecated; use VARCHAR(MAX) for new development. 
